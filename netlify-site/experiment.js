...
const participantInfo = {
  ...
  // Also capture any other custom URL params
...
timeline.push({
  type: jsPsychHtmlButtonResponse,
  stimulus: function () {
    // Save data as soon as the completion screen is shown
    saveDataToFirebase();
...
function saveDataToFirebase() {
  ...
  // Use Prolific PID as document ID if available, otherwise timestamp
  const sessionId = participantInfo.prolific_pid
  ...
  db.collection("music_rating_responses")
    ...
    .then(() => {
      ...
      if (DEBUG_MODE) {
        ...
        banner.textContent = "✅ DEBUG: Data saved to Firebase successfully! Doc ID: " + sessionId;
        document.body.prepend(banner);
